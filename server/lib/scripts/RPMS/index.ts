...
function getRequiredItems(householdMembers: IHouseholdMemberInfo[]): IMealBoxItem[] {
    //todo: add logic here
...
async function findCloseByDistributionCenters(pointGeometry: IPointGeometry, distanceInMeters?: any) {
...
async function handleMealBoxRequest(request: RequestMealBox) {
    ...
    try {
        await _mongooseTransactionAsync(async function () {
            let mealBox = await MealBoxModel.save();
            let closestDistributionCenters = await findCloseByDistributionCenters(request.location);
            if (closestDistributionCenters.length > 0) {
                closestDistributionCenters.forEach(async distributionCenterDocument => {
                    try {
                        ...
                        distributionCenter.addRequestToProcessQueue(mealBox);
                    }catch(error){
                        //todo: do something
                    }
                })
